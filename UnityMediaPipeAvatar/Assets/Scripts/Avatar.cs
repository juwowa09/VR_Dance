...
public class Avatar : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // animator.GetBoneTransform(HumanBodyBones.RightShoulder).localRotation *= Quaternion.Euler(new Vector3(0, 30, 0));
        ...
        if(parentCalibrationData.Count > 0)
        {
            ...
            transform.position = Vector3.Lerp(transform.position,initialPosition+ Vector3.up * displacement + Vector3.up * footGroundOffset,
                Time.deltaTime*5f);
            // 추가: hips 위치를 직접 위치 보정
            // Transform hips = animator.GetBoneTransform(HumanBodyBones.Hips);
            // Vector3 targetHipPosition = server.GetVirtualHip().position;
            // hips.position = Vector3.Lerp(hips.position, targetHipPosition, Time.deltaTime*45f);
        }
        ...
        if(parentCalibrationData.Count > 0)
        {
            ...
            Vector3 d = Vector3.Lerp(hipsTwist.initialDir, hipsTwist.CurrentDirection, .25f);
            d.y *= 0.5f;
